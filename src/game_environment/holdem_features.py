"""
Equity-based feature builder for reduced-deck Hold'em.
...
feature vectors following Bertsimas & Paskov (2022) methodology:
  ...
  - Hand bucket (categorical hand strength class)
  - Structural features (pocket pair, max hole rank, board pairing)
"""
...
def parse_holdem_state(state, player):
    """
    Extract game info from an OpenSpiel universal_poker state.

    Info state format example:
        [Round 0][Player: 0][Pot: 4][Money: 2147483646 2147483645]
        [Private: Kd9c][Public: ][Sequences: ]
    """
...
def opponent_aggression(sequence_str, player, round_idx, first_player=0):
    """
    Encode opponent aggression from a round's action sequence.
    Returns: 0.0 = no actions, 1.0 = passive, 2.0 = raised.
    """
    if not sequence_str:
        return 0.0
...
def build_features(state, player):
    """
    Build equity-based feature vector from a universal_poker game state.

    Returns numpy array of shape (20,) with interpretable features.
    """
    ...
    # 2-11. Future equity deciles (preflop only)
    ...
    # 17. Hand bucket (categorical strength class)
    bucket_name, bucket_value = classify_hand(
        info["hole_cards"], info["board_cards"], NUM_RANKS
    )
    ...
    # 19. Max hole card rank (normalized)
    if info["hole_cards"]:
    ...
    # 20. Board pairs one of my hole cards
